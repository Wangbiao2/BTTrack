...
class CrossAttention(nn.Module):
    def __init__(self, dim, num_heads=4, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        # Linear layers for Q from the decoder and K,V from the encoder
    ...
    def forward(self, query, key_value, return_attention=False):
        ...
        Args:
            ...
            key_value: Tensor of shape [batch_size, kv_len, embed_dim]
            return_attention: Whether to return attention weights along with output.
        Returns:
            Tensor or tuple: If `return_attention` is True, returns a tuple of (output, attention_weights),
                             otherwise returns only the output tensor.
        """
        B, N_q, C = query.shape
        B, N_kv, C = key_value.shape

        # Project the inputs into queries, keys, and values
        q = self.q(query).reshape(B, N_q, self.num_heads, C // self.num_heads).transpose(1, 2)  # (B, heads, N_q, C//heads)
