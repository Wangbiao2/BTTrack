...
class LTRTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, use_amp=False, grad_acc_steps=1):
        """
        args:
        ...
        self.accumulation_steps = grad_acc_steps
        if use_amp:
    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader, 1):
            ...
            if (self.accumulation_steps is None) or (self.accumulation_steps == 1):
                ...
                loss = loss / self.accumulation_steps  # Adjust loss for accumulation steps
                ...
                if loader.training:
                    ...
                    scaled_loss = loss
                    if self.use_amp:
                        scaled_loss = self.scaler.scale(scaled_loss)  # Apply scaling for AMP

                    scaled_loss.backward()

                    if (i % self.accumulation_steps == 0) or (i == len(loader)):
                        if self.use_amp:
                            self.scaler.unscale_(self.optimizer)
                            if self.settings.grad_clip_norm > 0:
